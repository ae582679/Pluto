...
class NXUSBDriverLoadingStateMachine {
...
class NXUSBViewModel: NSObject {

	// Your dext may not start in unloaded state every time. Add logic or states to check this.
	@Published private var state: NXUSBDriverLoadingStateMachine.State = .unloaded
	...
	public var dextLoadingState: String {
		switch state {
		case .unloaded:
			return "NXUSBSerialDriver isn't loaded."
		case .activating:
			return "Activating NXUSBSerialDriver, please wait."
		case .needsApproval:
			return "Please follow the prompt to approve NXUSBSerialDriver."
		case .activated:
			return "NXUSBSerialDriver has been activated and is ready to use."
		case .activationError:
			return "NXUSBSerialDriver has experienced an error during activation.\nPlease check the logs to find the error."
		}
	}
}

extension NXUSBViewModel: ObservableObject {

}

extension NXUSBViewModel {
	...
	func activateExtension(_ dextIdentifier: String) {
		...
		self.state = NXUSBDriverLoadingStateMachine.process(self.state, .activationStarted)
	}
...
#if os(macOS)
extension NXUSBViewModel: OSSystemExtensionRequestDelegate {
	...
	func requestNeedsUserApproval(_ request: OSSystemExtensionRequest) {
		...
		self.state = NXUSBDriverLoadingStateMachine.process(self.state, .promptForApproval)
	}
	...
	func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
		...
		self.state = NXUSBDriverLoadingStateMachine.process(self.state, .activationFinished)
	}
	...
	func request(_ request: OSSystemExtensionRequest, didFailWithError error: Error) {
		...
		self.state = NXUSBDriverLoadingStateMachine.process(self.state, .activationFailed)
	}
}
#endif
